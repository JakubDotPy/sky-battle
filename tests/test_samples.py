...
def test_leader_aims_ahead_of_a_crossing_target_rather_than_at_it():
    bot = _load("leader")
    crossing = contact(x=400.0, y=100.0, heading_deg=90.0, speed=8.0, bearing_deg=0.0, range=300.0)
    a = bot.act(make_state(planes=[plane(id=0, x=100.0, y=100.0, heading_deg=0.0, contacts=(crossing,))]))[0]
...
def test_wingman_remembers_a_contact_after_losing_it():
    ...
    bot.act(
        make_state(
            tick=1,
            planes=[
                plane(id=0, kind="scout", contacts=(seen,)),
...
def test_leader_beats_chaser_over_a_fixed_seed_set():
    ...
    for seed in range(1, 6):
        r = run_round([SAMPLES / "leader.py", SAMPLES / "chaser.py"], SQ, seed=seed, arena=ARENA)
